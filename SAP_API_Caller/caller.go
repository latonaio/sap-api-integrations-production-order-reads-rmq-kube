...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-production-order-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) General(manufacturingOrder string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": generalData, "function": "ProductionOrderGeneral"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": componentData, "function": "ProductionOrderComponent"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "ProductionOrderItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": operationData, "function": "ProductionOrderOperation"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": statusData, "function": "ProductionOrderStatus"})
	if err != nil {
		c.log.Error(err)
		return
	}
